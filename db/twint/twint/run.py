...
class Twint:
    ...
    async def run(self):
        ...
        if self.config.Username is not None and self.config.User_id is None:
            ...
            if isinstance(self.config.Username, str):
                fixedUsername = self.config.Username
            else:
                fixedUsername = self.config.Username[0]
            self.config.User_id = await get.User(fixedUsername, self.config, self.conn, True)
            if self.config.User_id is None:
                raise ValueError("Cannot find twitter account with name = " + fixedUsername)
    ...
    async def Lookup(self):
        ...
        try:
            if self.config.User_id is not None and self.config.Username is None:
                ...
                self.config.Username = await get.Username(self.config.User_id, self.config.Bearer_token,
                                                          self.config.Guest_token)
            await get.User(self.config.Username, self.config, self.conn)
